...
def main(
    ...
    topology_rng = torch.Generator(device=device)
    topology_rng.manual_seed(SEED + 20_003)
    ...
    for i_iter in pbar:
        ...
        rast_out, fragments = ds.rasterize_multires_triangle_alpha(
            ...
            return_fragments=True, rng_seed=SEED, rng_counter=i_iter,
        )
        feat = ds.multires_triangle_color(
        ...
        if lambda_normal_prior_effective > 0 or lambda_depth_prior_effective > 0:
            ...
            pixel_fragment_csr = ds.build_pixel_fragment_csr(
                fragments.frag_pix, batch_idx.shape[0], (H, W),
            )
            expected_surface = ds.vertex_expected_surface_samples(
                ...
                (H, W),
                pixel_fragment_csr=pixel_fragment_csr,
            )
            del batch_Tcw, batch_K, pixel_fragment_csr
